...
class PyreThemeFrameworkMetaboxes {
	...
	function admin_script_loader() {
		...
		if (is_admin() && ($pagenow=='post-new.php' || $pagenow=='post.php')) {
			...
			wp_register_script('avada_upload', get_template_directory_uri().'/js/upload.js', array('jquery'), $theme_info->get( 'Version' ));
	...
	public function add_meta_boxes()
	{
		...
		$this->add_meta_box('post_options', 'Fusion Options', 'post');

		$this->add_meta_box('page_options', 'Fusion Options', 'page');

		$this->add_meta_box('portfolio_options', 'Fusion Options', 'avada_portfolio');

		$this->add_meta_box('es_options', 'Elastic Slide Options', 'themefusion_elastic');

		$this->add_meta_box('woocommerce_options', 'Fusion Options', 'product');
