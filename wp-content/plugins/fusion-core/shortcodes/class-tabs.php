...
class FusionSC_Tabs {
	...
	function render_parent( $args, $content = '') {
		...
		for( $i = 0; $i < count( $this->tabs ); $i++ ) {
			...
			if( $is_first_tab ) {
				$html .= sprintf( '<li %s><a %s>%s%s</a></li>', FusionCore_Plugin::attributes( 'active' ), FusionCore_Plugin::attributes( 'tabs-shortcode-link', array( 'index' => $i ) ), 
								  $icon, $this->tabs[$i]['title'] );
				$is_first_tab = false;
			} else {
				$html .= sprintf( '<li><a %s>%s%s</a></li>', FusionCore_Plugin::attributes( 'tabs-shortcode-link', array( 'index' => $i ) ), 
								  $icon, $this->tabs[$i]['title'] );
			}			
		}
