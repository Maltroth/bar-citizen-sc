		...
		if( self::$args['group'] ) {
			...
			if( $query->have_posts() ):
				...
				while( $query->have_posts() ): $query->the_post();
					...
					$html .= sprintf( '<li>%s</li>', tf_content( self::$args['excerpt'], true ) . $caption );
		...
		if( $query ):
			...
			foreach( $query as $attachment ):
				...
				if( self::$args['lightbox'] == 'yes' ) {
					$output = sprintf( '<a href="%s" data-title="%s" data-caption="%s" title="%s" data-rel="prettyPhoto[flex_%s]">%s</a>', $image, $title, $alt, $title, $this->flex_counter, $image_output );
				}
