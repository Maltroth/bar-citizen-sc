...
class FusionSC_Blog {
	...
	function attr() {
		...
		$attr['class'] = sprintf( 'fusion-blog-shortcode fusion-blog-shortcode-%s fusion-blog-%s fusion-blog-%s', $this->blog_sc_counter, $blog_layout, self::$args['scrolling'] );

		if( self::$args['class'] ) {
			$attr['class'] .= ' ' . self::$args['class'];
		}

		if(self::$args['blog_grid_column_spacing'] == '0' || self::$args['blog_grid_column_spacing'] == '0px') {
	...
	function posts_container_attr() {
		...
		if ( self::$args['layout'] == 'grid' ) {
			 $attr['class'] .= sprintf( ' grid-layout grid-layout-%s isotope', self::$args['blog_grid_columns'] );

			 if( self::$args['blog_grid_column_spacing'] || self::$args['blog_grid_column_spacing'] === '0' ) {
			 	$attr['data-grid-col-space'] = self::$args['blog_grid_column_spacing'];
			 }
			 
			 $attr['style'] = sprintf( 'margin: 0 %spx;', ( -1 ) * self::$args['blog_grid_column_spacing'] / 2 );
		}
	...
	function wrap_loop_open() {
		...
		$wrapper = $class_sidebar = $class_timeline_icon = '';
		
		$sidebar_1 = get_post_meta( $post->ID, 'sbg_selected_sidebar_replacement', true );

		if( ( is_array( $sidebar_1 ) && ( $sidebar_1[0] || $sidebar_1[0] === '0' ) ) ||
			is_page_template( 'side-navigation.php' )
		) {
			$class_sidebar = ' timeline-sidebar-layout';
			$class_timeline_icon = ' has-sidebar';
		}
		...
		if( self::$args['layout'] == 'timeline' ) {
			...
			$wrapper = sprintf( '<div %s><i %s></i></div>', FusionCore_Plugin::attributes( 'timeline-icon' . $class_timeline_icon ), FusionCore_Plugin::attributes( 'fusionicon-bubbles' ) );
			$wrapper .= sprintf( '<div %s>', FusionCore_Plugin::attributes( 'blog-timeline-layout' . $class_sidebar ) );
	...
	function date_and_format() {
		...
		$inner_content .= sprintf( '</div>
									...
									</div>', FusionCore_Plugin::attributes( 'format-box' ), FusionCore_Plugin::attributes( 'fusionicon-'.$format_class ) );
	...
	function post_meta_data( $return_all_meta = false ) {
		...
		if( self::$args['meta_author'] ) {
			$inner_content .= sprintf( '%s <span %s><a %s>%s</a>' . '</span><span %s>|</span>', __( 'By', 'fusion-core' ), FusionCore_Plugin::attributes( 'blog-shortcode-meta-author' ), 
		...
		if( self::$args['meta_date'] ) {
			$updated = '';
			if( ! $smof_data['disable_date_rich_snippet_pages'] ) {
				$updated = sprintf( '<span %s>%s</span>', FusionCore_Plugin::attributes( 'blog-shortcode-meta-date-updated' ), get_the_modified_time( 'c' ) );
			}
		
			$inner_content .= sprintf( '<span %s>%s<time %s>%s</time></span><span %s>|</span>', FusionCore_Plugin::attributes( 'entry-time' ), $updated, FusionCore_Plugin::attributes( 'blog-shortcode-meta-date' ), get_the_time( $smof_data['date_format'] ), FusionCore_Plugin::attributes( 'meta-separator' ) );
		}
		...
		if( $return_all_meta ) {
			...
			if( self::$args['meta_categories'] ) {
				...
				if( self::$args['meta_tags'] ) {
					$output = __( 'Categories:', 'fusion-core' ) . ' ';
				}
				$count = 1;

				foreach( $categories as $category ) {

					$output .= '<a href="'.get_category_link( $category->term_id ).'" title="' . esc_attr( sprintf( __( "View all posts in %s", 'fusion-core' ), $category->name ) ) . '">'.$category->cat_name.'</a>';
			...
			if( self::$args['meta_tags'] ) {
				$inner_content .= $this->post_meta_tags();
			}
	...
	function grid_timeline_comments() {
		...
		if( self::$args['meta_comments'] ) {
			...
			$comments_icon = sprintf( '<i %s></i>&nbsp;', FusionCore_Plugin::attributes( 'fusionicon-bubbles' ) );
			ob_start();
	...
	function post_meta_tags() {
		...
		if( self::$args['meta_tags'] ) {
			...
			if( $tags ) {
				$inner_content = sprintf( '<span %s>%s %s</span><span %s>|</span>', FusionCore_Plugin::attributes( 'blog-shortcode-meta-tags' ), __( 'Tags:', 'fusion-core' ), $tags, FusionCore_Plugin::attributes( 'meta-separator' ) );
			}
	...
	function read_more() {
		...
		if( self::$args['meta_link'] ) {
			...
			if( self::$args['meta_read'] ) {
				...
				$inner_content .= sprintf( '<a href="%s">%s</a>', get_permalink(),  __( 'Read More', 'fusion-core' ) );
				$inner_content .= '</p>';
	...
	function entry_meta_attr() {
		...
		if( self::$args['layout'] == 'grid' || 
			...
			$attr['class'] = 'entry-meta-single' . $vcard;
		} else {
			$attr['class'] = 'entry-meta' . $vcard;
		}
