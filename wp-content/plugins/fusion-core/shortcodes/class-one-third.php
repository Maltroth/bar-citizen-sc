...
class FusionSC_OneThird {
	...
	function render( $args, $content = '') {
		...
		$html = sprintf( '<div %s><div %s>%s</div></div>%s', FusionCore_Plugin::attributes( 'one-third-shortcode' ), FusionCore_Plugin::attributes( 'one-third-shortcode-wrapper' ), do_shortcode( $content ), $clearfix );
	...
	function attr() {
		...
		$attr['class'] = 'fusion-one-third one_third fusion-layout-column fusion-column';
		
		if( self::$args['spacing'] == 'no' ) {
			$attr['style'] = 'width:33.3333%';
		}			
